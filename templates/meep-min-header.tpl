(define-param a 1e-6)                       ; length unit
(define-param pi 3.14159265358979)
(define-param c0 299792458)
(define-param plasmafreq 2.367)                ; plasma frequency of graphene
(define-param f 0.9472)                         ; Oscillator strengths
(define-param gamma 0.03857)                    ; Damping
(define norm (/ a (* c0 4.135666e-15)))   ; Normalization factor
(define frequency-p (* norm plasmafreq))
(define gamma1 (* norm 0.03857))
(define-param omega1 1e-3)                      ; frequency needs a small number but not zero
(define sigma-norm (/ (* f frequency-p frequency-p) (* omega1 omega1)))
(define mygraphene (make dielectric (epsilon 1)
    (E-susceptibilities
    (make lorentzian-susceptibility (frequency omega1) (gamma gamma1) (sigma sigma-norm))
    )))
